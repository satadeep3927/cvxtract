use super::loaders::{
    Document, DocumentLoader, FileType, Result, LoaderError,
    text::TextLoader,
    html::HtmlLoader,
    pdf::PdfLoader,
    docx::DocxLoader,
    utils::detect_file_type,
    utils::detect_from_bytes,
};
use std::path::Path;
...
impl UnstructuredLoader {
    ...
    pub fn load<P: AsRef<Path>>(&self, path: P) -> Result<Document> {
        ...
        if let Some(loader) = self.loaders.get(&content_type) {
            loader.load_from_bytes(&data, path.file_name().map(|s| s.to_string_lossy().to_string()).as_deref())
        } else {
            Err(LoaderError::UnsupportedFormat(
                format!("Unsupported file format: {:?}", content_type)
            ))
        }
    }
    
    /// Load a document from raw bytes with manual format specification
    pub fn load_from_bytes(&self, data: &[u8], format: FileType, filename: Option<&str>) -> Result<Document> {
        if let Some(loader) = self.loaders.get(&format) {
            loader.load_from_bytes(data, filename)
        } else {
            Err(LoaderError::UnsupportedFormat(
                format!("No loader available for format: {:?}", format)
            ))
        }
    }
    ...
    pub fn load_directory<P: AsRef<Path>>(&self, dir_path: P, recursive: bool) -> Result<Vec<Document>> {
...
impl UnstructuredLoader {
    ...
    pub fn extract_all_text<P: AsRef<Path>>(&self, dir_path: P, recursive: bool) -> Result<Vec<(String, String)>> {
        let documents = self.load_directory(dir_path, recursive)?;
        Ok(documents.into_iter().map(|doc| {
            let filename = doc.metadata.filename.unwrap_or_else(|| "unknown".to_string());
